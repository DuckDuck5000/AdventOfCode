...
def totalValidPasswords(longList):
    count = 0
    def isValid(low, high, letter, string):
    
        tempDict = Counter(string)
    
        if letter not in tempDict or tempDict[letter] > high or tempDict[letter] < low:
            return False
        return True
    def isValidPartTwo(low, high, letter, string):
    ...
    for line in longList:
        ...
        letter = temp[1][0]
        string = temp[-1]
        #if isValid(low, high, letter, string):
        #    count += 1
        if isValidPartTwo(low, high, letter, string):
            count += 1
    return count 

rawInput = """4-5 r: rrrjr
...
longList = rawInput.split("\n")

print(totalValidPasswords(longList))
